...
def calc_stability(result_df: pd.DataFrame) -> dict:
    ...
    # 과충·방전 방지율 (GRU 제어기는 blocked 없음 → SOC 근접 상황으로 계산)
    risky = (
...
def print_report(metrics: dict, title: str = "GRU BESS 평가 리포트"):
    sep = "=" * 62
    print(f"\n{sep}")
    print(f"  {title}")
    print(sep)
...
# 비교 출력 (Rule-Based vs GRU)
def print_comparison(rb_metrics: dict, gru_metrics: dict):
    """두 방식의 지표를 나란히 출력합니다 (논문 표 형식)."""
    sep = "=" * 70
    print(f"\n{sep}")
    print(f"  {'지표':<30} {'Rule-Based':>16} {'GRU':>16}")
    print(sep)

    def _row(label, rb_val, gru_val, fmt='{:.2f}'):
        rb_s   = fmt.format(rb_val)   if isinstance(rb_val,   float) else str(rb_val)
        gru_s = fmt.format(gru_val) if isinstance(gru_val, float) else str(gru_val)
        print(f"  {label:<30} {rb_s:>16} {gru_s:>16}")

    e_rb   = rb_metrics['economic'];   e_dl   = gru_metrics['economic']
    en_rb  = rb_metrics['energy'];     en_dl  = gru_metrics['energy']
    s_rb   = rb_metrics['stability'];  s_dl   = gru_metrics['stability']
    ...
    if 'prediction' in gru_metrics:
        p = gru_metrics['prediction']
        print("  [GRU 예측 성능]")
