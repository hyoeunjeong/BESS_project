...
def _load_from_api() -> tuple:
    ...
    if config.USE_WEATHER_FEATURES:
        # 기상 API 기반 실제 일사량 사용
        try:
            weather_df = api_client.fetch_weather_data(
                start, end, station_id=config.WEATHER_STATION_ID)
            solar_df = _solar_from_weather(weather_df)
        except Exception as ex:
            print(f"   [경고] 기상 API 실패 → 시뮬레이션 태양광 사용: {ex}")
            days = (pd.Timestamp(end) - pd.Timestamp(start)).days + 1
            solar_df = simulate_solar(days=days, start_date=start)
    else:
        days = (pd.Timestamp(end) - pd.Timestamp(start)).days + 1
